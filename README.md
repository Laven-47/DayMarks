...
DayMarks 是一个纯本地的 iOS 活动倒计时 App。
...
替换后，确保主 App 的 Signing & Capabilities 里启用了同一个 App Group。
...
活动数据写入 App Group 容器里的 `events.json`，封面图片写入同一容器下的 `Covers/` 目录。应用不会上传、同步或请求任何云端数据。
